...
export default function MoneyInSourcePieChart({ transactions }) {
  if (!transactions || transactions.length === 0) return null;
  // Aggregate by source
  const data = Object.values(
    transactions.reduce((acc, txn) => {
      acc[txn.source] = acc[txn.source] || { name: txn.source, value: 0 };
      acc[txn.source].value += Number(txn.amount || 0);
      return acc;
    }, {})
  );
  return (
